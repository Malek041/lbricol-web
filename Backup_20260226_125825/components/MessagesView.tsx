...
const MessagesView: React.FC<MessagesViewProps> = ({ orders, currentUser, initialSelectedJobId, isModal, onBackToOrders }) => {
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ marginBottom: '8px' }}>
                                    <span style={{ fontSize: '13px', fontWeight: 700, color: '#666', marginRight: '8px' }}>
                                        {conversation?.jobDates ? new Date(conversation.jobDates).toLocaleDateString(t({ en: 'en-US', fr: 'fr-FR' }), { day: 'numeric', month: 'short' }) : 'Flexible'}
                                    </span>
                                    <span style={{ fontSize: '16px', fontWeight: 900, color: '#000' }}>
                                        {conversation?.jobTime || 'Flexible'}
                                    </span>
                                    {conversation?.status === 'confirmed' && (
                                        <span style={{ fontSize: '13px', color: '#06C167', fontWeight: 700, marginLeft: '6px' }}>(On track)</span>
                                    )}
                                </div>
                        ...
                        {conversation?.jobDescription && (
                            <div style={{ marginTop: '12px', padding: '12px', backgroundColor: '#F9F9F9', borderRadius: '12px', border: '1px solid #F0F0F0' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                    ...
                                    <span style={{ fontSize: '11px', fontWeight: 900, color: '#333' }}>TASK DESCRIPTION</span>
                                </div>
