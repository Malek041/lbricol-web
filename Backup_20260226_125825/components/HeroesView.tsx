        ...
        <div className="flex flex-col h-full bg-[#FFFFFF] relative">
            ...
            <div className="pt-8 pb-6 px-6 bg-white sticky top-0 z-10 border-b border-neutral-100 shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
                <h1 className="text-[32px] font-black text-[#1D1D1D] tracking-tight flex items-center gap-3">
                    ...
                    {t({ en: 'My Heroes', fr: 'Mes Héros' })}
                </h1>
                <p className="text-[#6B6B6B] font-light text-[16px] mt-1">{t({ en: 'Re-book Bricolers you love', fr: 'Réservez à nouveau vos Bricolers préférés' })}</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 pb-24 space-y-4">
                {heroes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h2 className="text-[20px] font-black text-neutral-900 mb-2">{t({ en: 'No Heroes Yet', fr: 'Aucun Héros pour l\'instant' })}</h2>
                        <p className="text-neutral-500 text-[16px] font-medium max-w-[240px]">
                            {t({ en: 'Once you complete jobs with Bricolers, they will appear here.', fr: 'Une fois vos missions terminées avec des Bricolers, ils apparaîtront ici.' })}
                        </p>
                    </div>
                ) : (
                    heroes.map(hero => (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-1">
                                        ...
                                        <span className="text-[#6B6B6B] text-[13px] font-medium">• {hero.jobsCount} {t({ en: 'jobs', fr: 'missions' })}</span>
                                    </div>
                            ...
                            >
                                {t({ en: 'Book Slots', fr: 'Réserver des créneaux' })}
                            </button>
                        </motion.div>
                    ))
                )}
                        ...
                        >
                            ...
                            <div className="px-6 pt-6 pb-4 border-b border-[#E6E6E6] flex items-center justify-between sticky top-0 bg-white z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-center flex-1">
                                        <h3 className="text-[18px] font-black text-[#1D1D1D]">{t({ en: 'Book', fr: 'Réserver' })} {selectedHero.name}</h3>
                                        <p className="text-[12px] font-bold text-[#00A082] uppercase tracking-wider mt-0.5">
                                            {bookingStep === 1 && t({ en: '1. Date & Time', fr: '1. Date et Heure' })}
                                            {bookingStep === 2 && t({ en: '2. Choose Service', fr: '2. Choisir le service' })}
                                            {bookingStep === 3 && t({ en: '3. Task Details', fr: '3. Détails de la tâche' })}
                                            {bookingStep === 4 && t({ en: '4. Review & Confirm', fr: '4. Vérification et Confirmation' })}
                                        </p>
                                    </div>
                                    {bookingStep < 5 ? (
                            ...
                            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                                {isLoadingProfile && bookingStep < 5 ? (
                                    <div className="py-20 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-[#6B6B6B] font-medium text-[16px]">{t({ en: 'Loading provider availability...', fr: 'Chargement des disponibilités...' })}</p>
                                    </div>
                                ) : (
                                    <>
                                        {bookingStep === 1 && (
                                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                                <h4 className="text-[20px] font-bold text-[#1D1D1D]">{t({ en: 'When do you need them?', fr: 'Quand avez-vous besoin d\'eux ?' })}</h4>
                                                <div>
                                                    <label className="text-[14px] font-bold text-[#6B6B6B] block mb-2 uppercase">{t({ en: 'Select Date', fr: 'Choisir une date' })}</label>
                                                    <input
                                                        type="date"
                                                        value={selectedDate}
                                                ...
                                                <div>
                                                    <label className="text-[14px] font-bold text-[#6B6B6B] block mb-2 uppercase">{t({ en: 'Select Time', fr: 'Choisir une heure' })}</label>
                                                    <input
                                                        type="time"
                                                        value={selectedTime}
                                                    ...
                                                    {heroProfile && (
                                                        <p className="text-[12px] font-medium text-[#6B6B6B] mt-2 italic">
                                                            {t({ en: `Note: We'll verify this slot matches ${selectedHero.name}'s schedule.`, fr: `Note : Nous vérifierons que ce créneau correspond au planning de ${selectedHero.name}.` })}
                                                        </p>
                                                    )}
                                                </div>
                                            </motion.div>
                                        ...
                                        {bookingStep === 2 && (
                                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                                <h4 className="text-[20px] font-bold text-[#1D1D1D]">{t({ en: 'What do you need help with?', fr: 'De quoi avez-vous besoin ?' })}</h4>
                                                <div className="grid grid-cols-1 gap-3">
                                                    {(heroProfile?.services || selectedHero.services.map((s: string) => ({ serviceId: s }))).map((svc: any, idx: number) => {
                                                ...
                                                {selectedService && (
                                                    <div>
                                                        <label className="text-[14px] font-bold text-[#6B6B6B] block mb-2 mt-4 uppercase">{t({ en: 'Specific task (Optional)', fr: 'Tâche spécifique (optionnel)' })}</label>
                                                        <input
                                                            type="text"
                                                            value={selectedSubService}
                                                            onChange={e => setSelectedSubService(e.target.value)}
                                                            placeholder={t({ en: 'e.g. Paint bedroom, fix sink...', fr: 'ex. Peindre chambre, réparer lavabo...' })}
                                        ...
                                        {bookingStep === 3 && (
                                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                                <h4 className="text-[20px] font-bold text-[#1D1D1D]">{t({ en: 'Task Size & Details', fr: 'Taille de la tâche et détails' })}</h4>
                                                <div className="grid grid-cols-1 gap-3">
                                                    {TASK_SIZES.map(ts => (
                                                        <button
                                                            key={ts.id}
                                                        ...
                                                        >
                                                            ...
                                                            <div>
                                                                <div className="flex items-center justify-between mb-0.5">
                                                                    <span className={cn("text-[16px] font-bold", taskSize === ts.id ? "text-[#00A082]" : "text-[#1D1D1D]")}>{t(ts.label)}</span>
                                                                    <span className="text-[12px] font-bold text-[#6B6B6B]">{t(ts.estTime)}</span>
                                                                </div>
                                                                <p className="text-[13px] text-[#6B6B6B] leading-tight">{t(ts.desc)}</p>
                                                            </div>
                                                        </button>
                                                    ))}
                                                </div>
                                                ...
                                                <div>
                                                    <label className="text-[14px] font-bold text-[#6B6B6B] block mb-2 mt-4 uppercase">{t({ en: 'Description', fr: 'Description' })}</label>
                                                    <textarea
                                                        value={description}
                                                        onChange={e => setDescription(e.target.value)}
                                                        placeholder={t({ en: 'Any extra details for the hero?', fr: 'Des détails supplémentaires pour le héros ?' })}
                                                        rows={3}
                                        ...
                                        {bookingStep === 4 && (
                                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                                <div className="p-6 bg-[#F7F7F7] rounded-[24px] border border-[#E6E6E6]">
                                                    <h4 className="text-[20px] font-black text-[#1D1D1D] mb-4">{t({ en: 'Summary', fr: 'Résumé' })}</h4>

                                                    <div className="space-y-3">
                                                        <div className="flex items-start gap-3">
                                                            <Clock className="text-[#00A082] mt-0.5" size={20} />
                                                            <div>
                                                                <p className="text-[14px] font-bold text-[#6B6B6B]">{t({ en: 'Schedule', fr: 'Créneau' })}</p>
                                                                <p className="text-[16px] font-bold text-[#1D1D1D]">{selectedDate} {t({ en: 'at', fr: 'à' })} {selectedTime}</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-start gap-3">
                                                            <Star className="text-[#00A082] mt-0.5" size={20} />
                                                            <div>
                                                                <p className="text-[14px] font-bold text-[#6B6B6B]">{t({ en: 'Service', fr: 'Service' })}</p>
                                                        ...
                                                        <div className="flex items-start gap-3">
                                                            ...
                                                            <div>
                                                                <p className="text-[14px] font-bold text-[#6B6B6B]">{t({ en: 'Task Size', fr: 'Taille de la tâche' })}</p>
                                        ...
                                        {bookingStep === 5 && (
                                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                                                ...
                                                <h4 className="text-[24px] font-black text-[#1D1D1D]">{t({ en: 'Booking Sent!', fr: 'Réservation envoyée !' })}</h4>
                                                <p className="text-[#6B6B6B] text-[16px] max-w-[260px]">
                                                    {t({ en: `${selectedHero.name} has received your direct booking request. They will confirm shortly.`, fr: `${selectedHero.name} a reçu votre demande de réservation directe. Il/elle confirmera bientôt.` })}
                                                </p>
                                            </motion.div>
                                        )}
                                    </>
                            ...
                            {bookingStep > 0 && bookingStep < 5 && !isLoadingProfile && (
                                <div className="px-6 border-t border-[#E6E6E6] pt-4 pb-2">
                                    ...
                                    >
                                        {isSubmitting ? t({ en: 'Sending...', fr: 'Envoi...' }) : bookingStep === 4 ? t({ en: 'Confirm Booking', fr: 'Confirmer la réservation' }) : t({ en: 'Continue', fr: 'Continuer' })}
                                    </button>
                                </div>
                            )}
                        </motion.div>
