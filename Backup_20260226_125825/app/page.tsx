        ...
        localPros.forEach(p => {
          if (Array.isArray(p.services)) {
            p.services.forEach((s: any) => {
              ...
              if (subId) activeSubIds.add(subId);
            });
          }
        });

        setAvailableServices(Array.from(activeIds) as string[]);
        setAvailableSubServices(Array.from(activeSubIds) as string[]);
      } catch (err) {
      ...
      async (user) => {
        ...
        if (user) {
          ...
          unsubscribeJobs = onSnapshot(jobsQuery, (snapshot) => {
            ...
            const unratedDoneJob = loadedJobs.find(j => j.status === 'done' && !j.rated);
            if (unratedDoneJob) {
    ...
    try {
      ...
      for (const order of newOrders) {
        ...
        const jobData = {
          ...
          basePrice: parseFloat(price) ? (parseFloat(price) / 1.15) : 0,
          serviceFee: parseFloat(price) ? (parseFloat(price) - (parseFloat(price) / 1.15)) : 0,
                                  ...
                                  <div style={{ padding: '10px 0 20px' }}>
                                    ...
                                    {(() => {
                                      const services_list = activeTab === 'domestic' ? [
                                        'Handyman / small repairs', 'Furniture assembly', 'Cleaning', 'Glass cleaning', 'Plumbing', 'Electricity', 'Painting', 'Moving help', 'Appliance installation', 'Mounting (TV, shelves, curtains)', 'Errands & small deliveries', 'Gardening', 'Babysitting', 'Elderly assistance'
                                      ] : [
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                              {(service && (getServiceById(service)?.subServices?.filter(sub => availableSubServices.includes(sub.id)).length || 0) > 0 && !subService) ? (
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                  {sortedServices.slice(0, showAllServices ? undefined : 4).map((s, idx) => {
