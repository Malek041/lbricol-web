...
const ClientHome: React.FC<ClientHomeProps & { availableServiceIds?: string[] }> = ({
    ...
    const handleSearch = () => {
        if (searchText.trim()) onSelectService(searchText.trim());
    };
        ...
        <div className="min-h-screen bg-white flex flex-col pb-28">
            ...
            <div className="px-4 pb-4">
                <div className="flex items-center gap-2">
                    <div className="flex-1 flex items-center bg-white border border-neutral-200 rounded-full px-5 shadow-sm" style={{ height: 52 }}>
                        <input
                            type="text"
                            ...
                            className="flex-1 bg-transparent outline-none text-[15px] font-medium text-neutral-800 placeholder:text-neutral-400"
                        />
                    </div>
                    <button
                        onClick={handleSearch}
                        className="flex items-center justify-center rounded-full flex-shrink-0 shadow-md active:scale-95 transition-transform"
                        style={{ backgroundColor: G_GREEN, width: 52, height: 52 }}
                        aria-label="Search"
                    >
                    ...
                    <div className="px-4 pb-6 flex flex-wrap gap-2.5">
                        {active.subServices.map(sub => (
                            <motion.button
                                ...
                                className="px-4 py-2.5 rounded-full border border-[#E6E6E6] text-[15px] font-bold text-[#1D1D1D] bg-white hover:border-[#1D1D1D] active:bg-neutral-50 transition-all shadow-[0_2px_8px_rgba(0,0,0,0.03)]"
                            >
                                {sub}
                            </motion.button>
                        ))}
