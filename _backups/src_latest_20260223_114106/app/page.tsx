...
import OrderSubmissionFlow from '@/components/OrderSubmissionFlow';
import {
  MapPin,
  ChevronDown,
  Shield,
...
const Home = () => {
  ...
  const [mobileNavTab, setMobileNavTab] = useState<'home' | 'search' | 'heroes' | 'calendar' | 'messages' | 'profile' | 'share' | 'promocodes'>('home');
    ...
    const fetchActiveServices = async () => {
      try {
        ...
        const localPros = selectedArea
          ? pros.filter(p => p.workAreas?.includes(selectedArea))
          : pros;
        ...
        setAvailableServices(Array.from(activeIds) as string[]);
      } catch (err) {
  ...
  const handleWhatsAppSuccess = async (whatsappNumber: string) => {
    ...
    try {
      ...
      if (pendingQuickOrder) {
        ...
        await handleQuickOrderSubmit(data, whatsappNumber);
        return;
      }
  ...
  const handleQuickOrderSubmit = async (data: any, whatsappOverride?: string) => {
    ...
    setIsProgramming(true);
    try {
      const { service, subService, taskSize, description, bricolerId, city, area, date, time, totalPrice, basePrice, serviceFee } = data;
      ...
      showToast({ variant: 'error', title: "Failed to post mission." });
    } finally {
      setIsProgramming(false);
    }
  };
  ...
  return (
    <div style={{ backgroundColor: c.bg, color: c.text, minHeight: '100vh', transition: 'background-color 0.3s ease' }}>
      ...
      <main style={{
        ...
        height: isMobile && (mobileNavTab === 'calendar' || mobileNavTab === 'messages' || mobileNavTab === 'share' || mobileNavTab === 'promocodes') ? '100dvh' : 'auto'
      }}>
          ...
          <>
            ...
            {mobileNavTab === 'profile' && (
              <ProfileView
                ...
                onNavigate={(path) => {
                  if (path === '/orders') setMobileNavTab('calendar');
        ...
        {(!isMobile || mobileNavTab === 'home' || mobileNavTab === 'search') && (
          <>
                                  ...
                                  <div style={{ padding: '10px 0 20px' }}>
                                    ...
                                    {(() => {
                                      const services_list = activeTab === 'domestic' ? [
                                        'Handyman / small repairs', 'Furniture assembly', 'Cleaning', 'Glass', 'Plumbing', 'Electricity', 'Painting', 'Moving help', 'Appliance installation', 'Mounting (TV, shelves, curtains)', 'Errands & small deliveries', 'Gardening', 'Babysitting', 'Elderly assistance'
                                      ] : [
                                      ...
                                      const filteredServices = services_list.filter(s => {
                                        const svc = getServiceById(s);
                                        return svc ? cityServices.includes(svc.id) : cityServices.includes(s);
                                      });
