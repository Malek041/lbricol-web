...
import { getAllServices, getServiceById, getSubServiceName, type ServiceConfig } from '@/config/services_config';
import {
  FaHammer,
  FaScrewdriverWrench,
  FaBroom,
...
const Home = () => {
  ...
  const [mobileNavTab, setMobileNavTab] = useState<'home' | 'calendar' | 'messages' | 'profile'>('home');
  ...
  useEffect(() => {
    ...
    if (!savedLang) {
      ...
      // Migrate old city names (e.g. Marrakech vs Marrakesh or missing area suffix)
      let migratedCity = savedCity;
      if (migratedCity === 'Marrakesh') migratedCity = 'Marrakech';

      // If the city doesn't have an area suffix, default to (Inside) sparingly
      // This helps old sessions transition to the new suffixed document structure
      if (migratedCity && !migratedCity.includes(' (')) {
        migratedCity = `${migratedCity} (Inside)`;
    ...
    const checkAvailability = async () => {
      try {
        ...
        const eligibleProviders = providersSnap.docs.map(doc => ({ id: doc.id, ...doc.data() as any })).filter(data => {
          const sMatch = data.services?.find((s: any) => {
            const sId = typeof s === 'string' ? s : s.serviceId;
            return sId.toLowerCase() === service.toLowerCase();
          });
          if (!sMatch) return false;
          if (subService) {
            if (typeof sMatch === 'string') return true;
            return sMatch.subServices?.includes(subService);
          }
          return true;
        });
        ...
        const dateRangeQuery = query(
          collection(db, 'jobs'),
          where('city', '==', selectedCity),
          where('status', 'in', ['new', 'negotiating', 'pending', 'waiting', 'accepted', 'confirmed', 'in_progress'])
        );
  ...
  const handleWhatsAppSuccess = async (whatsappNumber: string) => {
    console.log("[handleWhatsAppSuccess] Saving WhatsApp number");
    ...
    try {
      ...
      // We pass it as override because state update is slow
      handleProgramOrder(undefined, whatsappNumber);
    } catch (error) {
      console.error("[handleWhatsAppSuccess] Error:", error);
      showToast({ variant: 'error', title: "Failed to save WhatsApp number.", description: "Please try again." });
    }
  };
  ...
  return (
    <div style={{ backgroundColor: c.bg, color: c.text, minHeight: '100vh', transition: 'background-color 0.3s ease' }}>
      <Header activeTab={activeTab} onTabChange={setActiveTab} isBricoler={isBricoler} user={currentUser} />
      <input
        type="file"
        ref={fileInputRef}
      ...
      <main style={{
        ...
        paddingBottom: isMobile ? ((mobileNavTab === 'calendar' || mobileNavTab === 'messages') ? '0' : '80px') : '0',
        overflow: isMobile && (mobileNavTab === 'calendar' || mobileNavTab === 'messages') ? 'hidden' : 'visible',
        height: isMobile && (mobileNavTab === 'calendar' || mobileNavTab === 'messages') ? '100dvh' : 'auto'
      }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        >
                          ...
                          <div style={{ fontSize: '16px', fontWeight: 500, color: service ? '#000' : '#5E5E5E', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {subService
                              ? `${service} › ${getSubServiceName(service, subService) || subService}`
                              : (service || t({ en: 'Select Service', fr: 'Choisir Service' }))
                            }
                          </div>
                        </motion.div>
                              ...
                              <div style={{ padding: '10px 0 20px' }}>
                                ...
                                {(() => {
                                  const services_list = activeTab === 'domestic' ? [
                                    'Handyman / small repairs', 'Furniture assembly', 'Cleaning', 'Plumbing', 'Electricity', 'Painting', 'Moving help', 'Appliance installation', 'Mounting (TV, shelves, curtains)', 'Errands & small deliveries', 'Gardening', 'Babysitting', 'Elderly assistance'
                                  ] : [
                                    'Car with driver', 'Car rental', 'Courier / delivery', 'Airport pickup', 'Intercity transport'
                                  ];

                                  const filteredServices = services_list.filter(s => cityServices.includes(s));
                                  ...
                                  return (
                                    <div style={{ width: '100%' }}>
                                      ...
                                      {loadingServices ? (
                                        ...
                                        <div style={{ padding: '1rem', textAlign: 'center', color: c.textMuted }}>
                                          {t({ en: 'No services available in this city yet.', fr: 'Aucun service disponible dans cette ville pour le moment.' })}
                                        </div>
                                      ) : (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                          {(service && (getServiceById(service)?.subServices?.filter(sub => citySubServices.includes(sub.id)).length || 0) > 0 && !subService) ? (
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                              ...
                                              >
                                                {getServiceById(service)?.subServices
                                                  .filter(sub => citySubServices.includes(sub.id))
                                                  .map((sub, sidx) => (
                                                    <motion.button
                                                      key={sub.id}
                                                      ...
                                                      style={{
                                                        ...
                                                        backgroundColor: '#FFF', border: `1px solid ${c.border}`, borderRadius: '10px',
                                                        padding: isMobile ? '12px 14px' : '14px 18px', cursor: 'pointer', width: '100%'
                                                      }}
                                                    >
                                                      <span style={{ fontWeight: 500, fontSize: '14px', color: '#000' }}>{sub.name}</span>
                                                      <ChevronDown style={{ transform: 'rotate(-90deg)', color: '#000' }} size={16} />
                                                    </motion.button>
                                                  ))}
                                              </motion.div>
                                            </div>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                              {filteredServices.slice(0, showAllServices ? undefined : 4).map((s, idx) => {
                                                ...
                                                return (
                                                  <button
                                                    ...
                                                    onClick={() => {
                                                      ...
                                                      if (config && config.subServices.some(sub => citySubServices.includes(sub.id))) {
                                                        setService(s);
                                                        setSubService(null);
                                                      } else {
                                                        setService(s);
                                                    ...
                                                    style={{
                                                      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                                                      backgroundColor: '#FFFFFF', border: `1px solid ${c.border}`, borderRadius: '10px',
                                                      padding: isMobile ? '14px 16px' : '16px 20px', width: '100%', cursor: 'pointer'
                                                    }}
                                                  >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                      <div style={{ fontSize: '18px', color: '#000' }}>{serviceIcons[s]}</div>
                                                      <span style={{ fontSize: '15px', fontWeight: 600, color: '#000' }}>{getTranslatedName(s, t)}</span>
                                                    </div>
                                                    <ChevronDown style={{ transform: 'rotate(-90deg)', color: '#000' }} size={18} />
                                                  </button>
                                                );
                                              })}
                                        ...
                                        return (
                                          <div
                                            ...
                                            style={{
                                              ...
                                              backgroundColor: isSelected ? '#000' : isInRange ? '#F0F0F0' : isToday ? '#F3F3F3' : 'transparent',
                                            ...
                                            return (
                                              <motion.button
                                                ...
                                                style={{
                                                  ...
                                                  backgroundColor: isTimeUnavailable ? '#EBEBEB' : (selectedTimes[activeSchedulingDate || selectedDates[0]] === t_val ? '#000' : '#F3F3F3'),
                              ...
                              return (
                                <div style={{ width: '100%' }}>
                                  ...
                                  {loadingServices ? (
                                    ...
                                    <div style={{ padding: '2rem', textAlign: 'center', color: c.textMuted, backgroundColor: '#F9F9F9', borderRadius: '16px' }}>
                                      {t({ en: 'No services available in this city yet.', fr: 'Aucun service disponible dans cette ville pour le moment.' })}
                                    </div>
                                  ) : (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
                                          ...
                                          >
                                            {getServiceById(service)?.subServices
                                              ...
                                              .map((sub, sidx) => (
                                                ...
                                                >
                                                  <span style={{ fontWeight: 600, fontSize: '14px', color: '#000' }}>{sub.name}</span>
                                                  <ChevronDown style={{ transform: 'rotate(-90deg)', color: '#000' }} size={16} />
                                                </motion.button>
                                              ))}
                                          </motion.div>
                                          ...
                                          return (
                                            <button
                                              ...
                                              style={{
                                                ...
                                                backgroundColor: '#FFFFFF', border: `1px solid ${c.border}`, borderRadius: '16px',
                                                padding: '24px', width: '100%', cursor: 'pointer', textAlign: 'left',
                                                transition: 'all 0.2s ease',
                                                boxShadow: '0 2px 10px rgba(0,0,0,0.03)'
                                              }}
                                            >
                                              <div style={{ fontSize: '24px', color: '#000' }}>{serviceIcons[s]}</div>
                                              <span style={{ fontSize: '16px', fontWeight: 700, color: '#000' }}>{getTranslatedName(s, t)}</span>
                                            </button>
                                          );
                                        })
                                      )}
          ...
          {viewingProviderId && providerDetails && (
            <div key="provider-details-modal" style={{ position: 'fixed', inset: 0, zIndex: 4000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} style={{ position: 'relative', backgroundColor: '#FFFFFF', width: '100%', maxWidth: '520px', borderRadius: '40px', padding: '2.5rem', boxShadow: '0 30px 90px rgba(0,0,0,0.12)', border: `1px solid #F0F0F0`, display: 'flex', flexDirection: 'column', gap: '2rem', maxHeight: '85vh', overflowY: 'auto' }}>
                ...
                <div style={{ marginTop: 'auto', paddingTop: '1rem' }}><button onClick={() => setViewingProviderId(null)} style={{ width: '100%', padding: '1.5rem', borderRadius: '24px', backgroundColor: '#000000', color: '#FFFFFF', border: 'none', fontSize: '16px', fontWeight: 950, cursor: 'pointer' }}>Close Profile</button></div>
              </motion.div>
            </div>
          )}
        </AnimatePresence>
