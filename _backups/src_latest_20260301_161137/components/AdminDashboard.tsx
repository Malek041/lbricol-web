...
const AdminDashboard = memo(({ t }: AdminDashboardProps) => {
    ...
    const { stats, loading } = useAdminDashboardStats(selectedCity);
        ...
        <div className="flex flex-col min-h-screen bg-[#FAFAFA] pb-24">
            ...
            <div className="bg-[#FFC244] pt-12 pb-24 px-5 rounded-b-[40px]">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <StatCard
                        title={t({ en: 'Total GMV', fr: 'Volume (GMV)', ar: 'إجمالي حجم العمليات' })}
                    ...
                    />
                    <StatCard
                        title={t({ en: 'Revenue', fr: 'Revenus', ar: 'الأرباح' })}
            ...
            <div className="px-5 -mt-12 space-y-4">
                ...
                <div className="bg-white p-6 rounded-[32px] shadow-sm border border-neutral-100">
                    <h3 className="text-lg font-black text-black mb-4">
                        {t({ en: 'Platform Activity', fr: 'Activité Plateforme', ar: 'نشاط المنصة' })}
                    </h3>
                        ...
                        {[1, 2, 3].map((_, i) => (
                            <div key={i} className="flex items-center gap-4 p-4 bg-neutral-50 rounded-2xl">
                                ...
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-black">{t({ en: 'New order in Marrakech', fr: 'Nouvelle commande à Marrakech', ar: 'طلب جديد في مراكش' })}</p>
                                    <p className="text-xs text-neutral-500">{t({ en: '2 minutes ago', fr: 'Il y a 2 minutes', ar: 'منذ دقيقتين' })}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-black text-black">450 MAD</p>
                                </div>
                            </div>
                        ))}
                    </div>
