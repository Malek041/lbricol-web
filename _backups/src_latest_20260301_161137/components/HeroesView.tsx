...
export default function HeroesView({ orders }: HeroesViewProps) {
    ...
    const handleBookHero = async (hero: HeroData) => {
        ...
        // Pre-select today or tomorrow as default date
    ...
    const uploadImageToStorage = async (file: File, path: string): Promise<string> => {
        ...
        const result = await uploadStorageBytes(storageRef, file, { contentType: 'image/jpeg' });
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-1">
                                        ...
                                        <span className="text-[#6B6B6B] text-[13px] font-medium">• {hero.jobsCount} {t({ en: 'jobs', fr: 'missions', ar: 'مهام' })}</span>
                                    </div>
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    className={cn(
                                                                        ...
                                                                        isSelected ? "bg-[#00A082] border-[#00A082] shadow-[0_8px_16px_rgba(0,160,130,0.25)]" : "bg-white border-[#F0F0F0] hover:border-[#008C74]/50"
                                                                    )}
                                                                >
                                                ...
                                                <div className="bg-[#FAFAFA] rounded-[32px] p-8 border border-neutral-100">
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex justify-between items-center px-1">
                                                            ...
                                                            <span className="text-[16px] font-black text-[#1D1D1D]">{Math.round((heroProfile?.hourlyRate || 100) * (TASK_SIZES.find(s => s.id === taskSize)?.duration || 1) * 0.85)} MAD</span>
                                                        </div>
                                                        <div className="flex justify-between items-center px-1">
                                                            <span className="text-[16px] font-bold text-[#6B6B6B]">{t({ en: 'Service Fee', fr: 'Commission', ar: 'عمولة الخدمة' })}</span>
                                                            <span className="text-[16px] font-black text-[#1D1D1D]">{Math.round((heroProfile?.hourlyRate || 100) * (TASK_SIZES.find(s => s.id === taskSize)?.duration || 1) * 0.15)} MAD</span>
                                                        </div>
                                                        <div className="flex justify-between items-center px-1 pt-4 border-t border-neutral-200">
                                                            ...
                                                            <span className="text-[22px] font-black text-[#00A082]">{Math.round((heroProfile?.hourlyRate || 100) * (TASK_SIZES.find(s => s.id === taskSize)?.duration || 1))} MAD</span>
                                                        </div>
                                                    </div>
                                                </div>
