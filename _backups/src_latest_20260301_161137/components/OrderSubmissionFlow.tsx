...
const OrderSubmissionFlow: React.FC<OrderSubmissionFlowProps> = ({
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Show local previews immediately for UX responsiveness
        const previews = await Promise.all(toUpload.map(file =>
        ...
        if (!user) {
            // Not logged in — keep base64 previews. They'll be visible but not persisted to order images.
                                        ...
                                        <div className="mb-4">
                                            <div className="grid grid-cols-2 gap-3">
                                                ...
                                                <div className="bg-neutral-50 rounded-2xl p-4 flex items-center gap-3 border border-neutral-100/50">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider">{t({ en: 'Total', fr: 'Total' })}</span>
                                                        <span className="text-[15px] font-black text-black">{Math.round((selectedPro?.hourlyRate || 100) * (activeTaskSize?.duration || 1))} MAD</span>
                                                    </div>
                                                </div>
                                    ...
                                    <div className="flex flex-col gap-6">
                                        <div className="flex justify-between items-center px-2">
                                            ...
                                            <div className="flex flex-col items-end">
                                                {applyReferralDiscount && referralDiscountAvailable > 0 && (
                                                    <span className="text-[14px] font-bold text-neutral-400 line-through">
                                                        {Math.round((selectedPro?.hourlyRate || 100) * (activeTaskSize?.duration || 1))} MAD
                                                    </span>
                                                )}
                                                <span className="text-[28px] font-black text-black tracking-tighter" style={{ color: applyReferralDiscount && referralDiscountAvailable > 0 ? '#00A082' : 'black' }}>
                                                    {Math.max(0, Math.round((selectedPro?.hourlyRate || 100) * (activeTaskSize?.duration || 1)) - (applyReferralDiscount ? referralDiscountAvailable : 0))} MAD
                                                </span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
