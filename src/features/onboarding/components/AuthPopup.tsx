...
const AuthPopup = ({ isOpen, onClose, onSuccess }: AuthPopupProps) => {
    ...
    const isValid = validatePhone(whatsapp, selectedCountry);
    ...
    const handleWhatsAppSubmit = async () => {
        if (!tempUser || !isValid) return;
        setIsLoading(true);
        try {
            const userRef = doc(db, 'users', tempUser.uid);
            const clientRef = doc(db, 'clients', tempUser.uid);

            const e164 = formatToE164(whatsapp, selectedCountry.dialCode);
                        ...
                        <AnimatePresence mode="wait">
                            {!showWhatsAppRequest ? (
                                ...
                                >
                                    ...
                                    <h2 style={{ fontSize: titleSize, fontWeight: 700, marginBottom: '12px', color: '#111827' }}>{t({ en: 'Final Step', fr: 'Dernière étape', ar: 'الخطوة الأخيرة' })}</h2>
                                    ...
                                    <div style={{ 
                                        ...
                                        borderRadius: '16px',
                                        padding: '0 16px',
                                        border: `2px solid ${isValid ? '#0CB380' : whatsapp.length > 0 ? '#FF5252' : '#E5E7EB'}`,
                                        height: '70px',
                                        alignItems: 'center',
                                        ...
                                        <input
                                            ...
                                            autoFocus
                                            style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', outline: 'none', fontSize: '18px', fontWeight: 700, color: '#111827' }}
                                        />
                                    </div>
                                    ...
                                    <motion.button
                                        ...
                                        style={{
                                            ...
                                            padding: '20px',
                                            borderRadius: '100px',
                                            background: (isLoading || !isValid) ? '#F0F0F0' : '#01A083',
                                            color: (isLoading || !isValid) ? '#B3B3B3' : '#fff',
                                            fontSize: '18px',
                                            fontWeight: 900,
                                            border: 'none',
                                    ...
                                    >
                                        {isLoading ? t({ en: 'Saving...', fr: 'Enregistrement...', ar: 'جاري الحفظ...' }) : t({ en: 'Complete & Continue', fr: 'Finaliser et continuer', ar: 'إكمال ومتابعة' })}
                                    </motion.button>
                                </motion.div>
                            )}
                        </AnimatePresence>
