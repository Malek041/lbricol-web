...
const Header = ({ activeTab, onTabChange, isBricoler = false, user = null }: HeaderProps) => {
    ...
    const mobileHeaderVerticalPadding = `${Math.round(fluidMobilePx(viewportWidth, 4, 7))}px`;
    const mobileHeaderHorizontalPadding = `${Math.round(fluidMobilePx(viewportWidth, 12, 16))}px`;
    ...
    return (
        <header style={{
            ...
            <div style={{
                maxWidth: '1280px',
                margin: '0 auto',
                padding: isMobile ? `${mobileHeaderVerticalPadding} ${mobileHeaderHorizontalPadding}` : '0.5rem 1.5rem',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between'
            }}>
                ...
                {/* Center: Tabs — hidden on mobile (logo shown in ClientHome instead) */}
                <div style={{
                    flex: 1,
                    ...
                    <div style={{
                        ...
                        {[
                            { id: 'domestic', label: t({ en: 'Domestic Help', fr: 'Aide Ménagère', ar: 'مساعدة منزلية' }), icon: Home },
                            { id: 'go', label: t({ en: 'Mobility & Logistics', fr: 'Mobilité & Logistique', ar: 'نقل ولوجستيك' }), icon: Truck }
                        ].map(tab => {
                ...
                <div style={{
                    ...
                    gap: isMobile ? (isCompactPhone ? '0.35rem' : '0.5rem') : '1rem',
                    flex: 1
                }}>
                    {!isMobile && (
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            ...
                            >
                                {t({ en: 'Download App', fr: 'Télécharger l\'App', ar: 'تحميل التطبيق' })}
                            </button>
                            <button
                                onClick={() => {
                                    if (isBricoler) {
                            ...
                            >
                                {isBricoler ? t({ en: 'Switch to Bricoler Mode', fr: 'Passer en Mode Bricoleur', ar: 'التبديل إلى وضع الحرفي' }) : t({ en: 'Become a Bricoler', fr: 'Devenir un Bricoleur', ar: 'كن حرفياً' })}
                            </button>
                        </div>
                    )}
                                ...
                                <AnimatePresence>
                                    {isMenuOpen && (
                                        ...
                                        >
                                            ...
                                            <MenuOption
                                                label={isBricoler ? t({ en: 'Switch to Bricoler Mode', fr: 'Passer en Mode Bricoleur', ar: 'التبديل إلى وضع الحرفي' }) : t({ en: 'Become a Bricoler', fr: 'Devenir un Bricoleur', ar: 'كن حرفياً' })}
                                                sublabel={isBricoler ? t({ en: 'Manage your profile and jobs.', fr: 'Gérez votre profil et vos missions.', ar: 'إدارة ملفك الشخصي ومهامك.' }) : t({ en: 'Start earning extra income.', fr: 'Commencez à gagner un revenu extra.', ar: 'ابدأ في كسب دخل إضافي.' })}
                                                onClick={() => {
                                                    if (isBricoler) {
                                                        window.location.href = '/provider';
                                                    } else {
                                            ...
                                            {user ? (
                                                <MenuOption
                                                    icon={LogOut}
                                                    label={t({ en: 'Log out', fr: 'Se déconnecter', ar: 'تسجيل الخروج' })}
                                                    onClick={() => {
                                                        signOut(auth);
                                                        setIsMenuOpen(false);
                                                    }}
