        ...
        switch (service) {
            ...
            case 'errands':
                return {
                    ...
                    options: [
                        { id: 'small', duration: 1, label: { en: 'Quick Errand / 1 Stop', fr: 'Course rapide / 1 Arrêt', ar: 'مشوار سريع / توقف واحد' }, estTime: { en: 'Est: 1 hr', fr: 'Est: 1h', ar: 'حوالي ساعة' }, desc: { en: 'Quick pickup, drop-off or short task.', fr: 'Retrait rapide, dépôt ou petite tâche.', ar: 'توصيل أو استلام سريع أو مهمة قصيرة.' }, icon: '/Images/Location&taskSize_OrderSetup/TaskSizes/SmallTask.png' },
                        { id: 'medium', duration: 3, label: { en: 'Multiple Stops / Shopping', fr: 'Plusieurs Arrêts / Achats', ar: 'عدة توقفات / تسوق' }, estTime: { en: 'Est: 2-3 hrs', fr: 'Est: 2-3h', ar: '2-3 ساعات' }, desc: { en: 'Grocery shopping or hitting multiple locations.', fr: 'Courses ou passage à plusieurs endroits.', ar: 'تسوق أو الذهاب إلى عدة أماكن.' }, icon: '/Images/Location&taskSize_OrderSetup/TaskSizes/MediumSize.png' },
                        { id: 'large', duration: 5, label: { en: 'Half Day / Many Tasks', fr: 'Demi-journée / Nombreuses tâches', ar: 'نصف يوم / مهام عديدة' }, estTime: { en: 'Est: 4+ hrs', fr: 'Est: 4h+', ar: 'أكثر من 4 ساعات' }, desc: { en: 'Extensive list of errands or waiting in lines.', fr: 'Longue liste de courses ou file d\'attente.', ar: 'قائمة طويلة من المشاوير أو الانتظار في طوابير.' }, icon: '/Images/Location&taskSize_OrderSetup/TaskSizes/BigTask.png' },
                    ]
                };
            case 'plumbing':
            case 'electricity':
                                                            ...
                                                            <div className="flex-1">
                                                                <div className="flex items-center justify-between mb-2">
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        {idx === 1 && (
                                                                            <span className="px-2 py-0.5 rounded-full bg-[#FFEAC2] text-[#E09000] text-[10px] font-black uppercase tracking-wider">
                                                                                -5% {t({ en: 'Discount', fr: 'Promo', ar: 'خصم' })}
                                                                            </span>
                                                                        )}
                                                                        {idx === 2 && (
                                                                            <span className="px-2 py-0.5 rounded-full bg-[#E6F6F2] text-[#00A082] text-[10px] font-black uppercase tracking-wider">
                                                                                -10% {t({ en: 'Discount', fr: 'Promo', ar: 'خصم' })}
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-[15px] font-black text-black">
                                                            {(() => {
                                                                ...
                                                                return Math.round((selectedPro?.hourlyRate || 100) * (activeTaskSize?.duration || 1) * multiplier);
                                                            })()} MAD
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                {referralDiscountAvailable > 0 && (
                                                    <div className="flex justify-between items-center mt-6 p-4 bg-[#E6F6F2] rounded-[16px] border border-[#00A082]/20">
                                                        ...
                                                        <button
                                                            ...
                                                            className={cn(
                                                                "px-5 py-2.5 rounded-full text-[14px] font-bold transition-all active:scale-95 border",
                                                                applyReferralDiscount
                                                                    ? "bg-[#00A082] text-white border-[#00A082]"
                                                                    : "bg-white text-[#00A082] border-[#00A082]/30 hover:border-[#00A082]"
                                                            )}
                                                        >
                                                            {applyReferralDiscount
                                                                ? t({ en: 'Applied', fr: 'Appliqué', ar: 'تم التطبيق' })
                                                                : t({ en: 'Apply', fr: 'Appliquer', ar: 'تطبيق' })}
                                                        </button>
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    <div className="flex flex-col gap-6">
                                        <div className="flex justify-between items-center px-2">
                                            ...
                                            <div className="flex flex-col items-end">
                                                {(() => {
                                                    const baseCalc = Math.round((selectedPro?.hourlyRate || 100) * (activeTaskSize?.duration || 1));
                                                    const sizeIndex = serviceConfig.options.findIndex(s => s.id === taskSize);
                                                    let multiplier = 1;
                                                    if (sizeIndex === 1) multiplier = 0.95;
                                                    else if (sizeIndex === 2) multiplier = 0.9;
