...
/**
 * Helper to check restocking thresholds and dispatch a courier if needed
 */
async function checkRestockingThresholds(propertyId, stockReport) {
  const db = admin.firestore();
  
  const propertyDoc = await db.collection("properties").doc(propertyId).get();
  if (!propertyDoc.exists) return;
  const property = propertyDoc.data();
  const errandsDetails = property.automation?.errandsDetails;
  
  if (!errandsDetails || !stockReport) return;

  const itemsToRestock = [];
  const checklists = errandsDetails.checklists || {};

  // Flatten checklists to compare with stockReport
  Object.values(checklists).flat().forEach(item => {
    const reportItem = stockReport.find(r => r.name === item.name);
    if (reportItem && reportItem.quantity <= item.minStock) {
      const quantityToBuy = item.maxStock - reportItem.quantity;
      if (quantityToBuy > 0) {
        itemsToRestock.push({
          name: item.name,
  ...
  if (itemsToRestock.length > 0) {
    ...
    await db.collection("jobs").add({
      ...
      status: 'new',
      service: 'errands',
      subService: 'restocking_delivery',
      subServiceDisplayName: 'Réapprovisionnement (Coursier)',
      address: property.specs?.address || '',
      date: dateStr,
      time: "10:00",
      items: itemsToRestock,
      preferredSupplier: errandsDetails.preferredSupplier || null,
      isHostJob: true,
      isAutoGenerated: true,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
    });

    console.log(`Dispatched restocking courier for property ${propertyId} with ${itemsToRestock.length} items.`);
  }
}

/**
 * Trigger: On Job Updated
 * Handles restocking logic when a job is completed
 */
exports.onJobUpdated = onDocumentUpdated("jobs/{jobId}", async (event) => {
  ...
  if (after.status === 'completed' && before.status !== 'completed') {
    // If the job included a stock report, check thresholds
    if (after.stockReport && after.propertyId) {
      await checkRestockingThresholds(after.propertyId, after.stockReport);
    }
  }
});
...
/**
 * Trigger: On Planning Event Created
 * Dispatches jobs based on property automation settings
 */
exports.onPlanningCreated = onDocumentCreated("properties/{propertyId}/Planning/{jobId}", async (event) => {
  const data = event.data.data();
  const propertyId = event.params.propertyId;
  const type = data.type; // e.g. 'checkout', 'checkin', 'cleaning', 'stock_audit'
  ...
  // if (!automation.enabled) return; // For host, it might be always enabled if configured

  const serviceId = type === 'stock_audit' ? 'errands' : 'cleaning';
  
  await dispatchHostJob(propertyId, serviceId, type, data.date, data.time || "11:00");
});

/**
 * Trigger: On Calendar Event Created (Legacy Support)
 */
exports.onCalendarEventCreated = onDocumentCreated("properties/{propertyId}/calendar/{eventId}", async (event) => {
  ...
  if (eventType === 'Checkout' || eventType === 'checkout') {
    if (automation.services?.includes('cleaning')) servicesToDispatch.push('cleaning');
    if (automation.errandsDetails) servicesToDispatch.push('errands');
  } else if (eventType === 'Check-in' || eventType === 'checkin') {
